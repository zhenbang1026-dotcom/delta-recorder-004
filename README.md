...
| **角度三模式** | **Legacy**（旧颜色轮廓）+ **TEXT**（MAP 校准箭头）+ **Fusion**（三色精准主观测 + Legacy 连续性选择）；**截图方式不变** |
...
- **Fusion**：精准主观测会自动识别绿色、蓝色、黄色，并分别使用独立标定。切换新颜色需连续两帧确认，确认首帧不输出陈旧精准角；精准识别失败、质量不足或与 Legacy 冲突时，沿用现有 Legacy 降级与连续性选择。

新增 Fusion 三色精准识别不会改变 Legacy 与现有 TEXT 的行为，程序默认仍为 Legacy。
