...
def 计算自适应转向角度(角度差: float) -> float:
    if 是否text角度模式():
        # text 读角准 + 33.3 已知：大转向一次多拧一点，减少多次 turn_wait
        # 仍留 8% 余量，避免打满过冲
        幅度 = min(abs(角度差) * 0.92, 55.0)
...
def 普通模式参数() -> 模式参数:
    if 是否text角度模式():
        # text：少停车大转向，多边跑边修（日志：转向 24%→目标压到接近 legacy）
        return 模式参数(大角度阈值=28, 小角度阈值=8, 允许疾跑=True, 精准缩放=1.0)
    return 模式参数(大角度阈值=18, 小角度阈值=6, 允许疾跑=True, 精准缩放=1.0)


def 精准模式参数() -> 模式参数:
    if 是否text角度模式():
        return 模式参数(大角度阈值=18, 小角度阈值=5, 允许疾跑=False, 精准缩放=0.45)
    return 模式参数(大角度阈值=10, 小角度阈值=3, 允许疾跑=False, 精准缩放=0.35)


def text微调鼠标像素(角度差: float, 精准缩放: float = 1.0) -> int:
    """text 边跑边修：按 33.3 px/°，单帧最多约 16°，避免疾跑中拧飞。"""
    限幅 = max(-16.0, min(16.0, 角度差)) * 精准缩放
    return 角度差转鼠标像素(限幅)


def 读取路径(路径文件: str) -> list[路径点]:
    ...
    return 抽稀自动路线(结果) if 自动路线 else 结果
...
def 选择动作(*, 距离: int, 角度差: float, 到点阈值: int, 参数: 模式参数, 自动路线: bool = False) -> 动作指令:
    ...
    # text：中等角差一律边跑边修（限幅），少触发阻塞转向
    if 是否text角度模式():
        if 参数.允许疾跑 and 距离 >= 到点阈值 * 2:
            if abs(角度差) <= 参数.小角度阈值:
                return 动作指令("疾跑前进")
            return 动作指令("疾跑前进并微调", 鼠标像素=text微调鼠标像素(角度差, 参数.精准缩放))
        if abs(角度差) <= 参数.小角度阈值:
            return 动作指令("前进")
        return 动作指令("前进并微调", 鼠标像素=text微调鼠标像素(角度差, 参数.精准缩放))
...
class 实时定位器:
    ...
    def _稳定角度(self, angle: float) -> float:
        ...
        if 最近状态 is None:
            return float(angle)
        上次角度 = float(最近状态[2])
        if abs(计算最短角度差(上次角度, angle)) > 最大单帧角度跳变:
            return 上次角度
        return float(angle)
    ...
    def _确认角度跳变(self, x: int, y: int, angle: float, 复查函数) -> float:
        ...
        if 位移 > 角度跳变小位移距离 or 跳变 < 角度跳变复查阈值:
            return self._稳定角度(angle)
        复查角度 = float(复查函数())
        复查跳变 = abs(计算最短角度差(上次角度, 复查角度))
        if abs(计算最短角度差(angle, 复查角度)) <= 角度跳变复查一致阈值:
...
class 巡航控制器:
    ...
    def _转向后确认状态(
        ...
        是否收敛 = False
        拒绝原因 = ""
        # text 读角准 + 33.3 已知：缩短确认窗，少卡在 turn_wait
        超时 = 0.35 if 是否text角度模式() else 转向后确认超时秒数
        最小变化 = 2.5 if 是否text角度模式() else 转向后最小角度变化
        变差阈值 = 14.0 if 是否text角度模式() else 转向后异常变差角度
            ...
            while 当前索引 < len(self.路径点列表):
                ...
                if 距离 <= self.到点阈值:
                    ...
                    if self.记录器 is not None:
                        self.记录器.记录(坐标=f"{x},{y}", 角度=round(当前角度, 2), 索引=当前索引, 目标=f"{当前点.x},{当前点.y}", 距离=距离, 动作=切点动作)
                ...
                if 是否卡住:
                    ...
                    if self.记录器 is not None:
                        self.记录器.记录(坐标=f"{x},{y}", 角度=round(当前角度, 2), 索引=当前索引, 目标=f"{当前点.x},{当前点.y}", 距离=距离, 动作=脱困动作.类型, 鼠标像素=脱困动作.鼠标像素, 识别诊断=getattr(self.定位器, "识别诊断状态", ""))
                        self.记录器.保存事件截图("脱困")
                    self.执行器.执行(脱困动作)
                    self._上次脱困时间 = 当前时间
                    continue
...
def 巡航(
    ...
    控制器 = 巡航控制器(
        路径点列表=路径点列表,
        定位器=定位器 or 实时定位器(),
        执行器=Win32执行器() if 停止事件 is None else Win32执行器(停止事件=停止事件),
        到点阈值=到点阈值,
        参数=参数,
        终点对正=终点对正,
        日志函数=日志函数,
...
class Win32执行器:
    def __init__(self, 输入模块=win32_input, 停止事件: threading.Event | None = None):
        self.输入模块 = 输入模块
        self.停止事件 = 停止事件
        self._正在前进 = False
        self._本段已疾跑 = False
