...
TEXT_补采次数 = 2
TEXT_命令偏离阈值 = 40.0
# 角差大时更信读数，角差小时才和命令对半揉（避免欠纠时被命令拖住）
TEXT_读数融合权重 = 0.55
...
class 实时定位器:
    ...
    def _text融合读角(self, 读数角: float) -> float:
        ...
        if 命令角 is None:
            self._text命令角 = 读数角
            return 读数角
        偏离 = abs(计算最短角度差(float(命令角), 读数角))
        if 偏离 > 55.0:
            # 野值：信命令
            融合 = float(命令角) % 360.0
        elif 偏离 > 25.0:
            # 中等分歧：更信读数，避免欠纠被命令拖住
            融合 = text融合控制角(float(命令角), 读数角, 0.75)
        else:
            融合 = text融合控制角(float(命令角), 读数角, TEXT_读数融合权重)
        self._text命令角 = 融合
        try:
