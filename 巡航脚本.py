...
每度像素 = 100 / 3
# text 模式锁定该值（标定尺：33.3px=1°）；控制时再乘增益，避免一帧拧太多
TEXT_每度像素 = 100 / 3
# 边跑边修：用标定的一部分，单帧限幅小 → 像人微调
TEXT_微调像素增益 = 0.45
TEXT_微调最大角度 = 7.0
# 阻塞转向：略保守（0.7×角差），单次最多约 28°
TEXT_转向像素增益 = 0.70
TEXT_单次最大转向角度 = 28.0
# 稳采样减负
TEXT_转向后稳定等待 = 0.05
...
def 计算自适应转向角度(角度差: float) -> float:
    if 是否text角度模式():
        # 保守：单次最多约 28°，避免“滑太猛”
        幅度 = min(abs(角度差) * 0.75, TEXT_单次最大转向角度)
...
def text微调鼠标像素(角度差: float, 精准缩放: float = 1.0) -> int:
    """text 边跑边修：小步多次。默认 ≤7°×0.45 增益 ≈ 约 105 像素/帧。"""
    限幅 = max(-TEXT_微调最大角度, min(TEXT_微调最大角度, 角度差)) * 精准缩放
    return 角度差转鼠标像素(限幅, TEXT_微调像素增益)


def text中位角度(角度列表: list[float]) -> float:
    """圆形中位数（0–360）。"""
    if not 角度列表:
        return 0.0
    弧 = [math.radians(a) for a in 角度列表]
    sx = sum(math.cos(a) for a in 弧) / len(弧)
    sy = sum(math.sin(a) for a in 弧) / len(弧)
...
class 实时定位器:
    ...
    def _稳定角度(self, angle: float) -> float:
        ...
        上限 = 55.0 if 是否text角度模式() else 最大单帧角度跳变
    ...
    def _确认角度跳变(self, x: int, y: int, angle: float, 复查函数) -> float:
        ...
        复查阈 = 32.0 if 是否text角度模式() else 角度跳变复查阈值
        if 位移 > 角度跳变小位移距离 or 跳变 < 复查阈:
            return self._稳定角度(angle)
        复查角度 = float(复查函数())
        复查跳变 = abs(计算最短角度差(上次角度, 复查角度))
        一致阈 = 12.0 if 是否text角度模式() else 角度跳变复查一致阈值
...
class 巡航控制器:
    ...
    def _转向后确认状态(
        ...
        if 最新可信状态 is not None:
            self._待处理状态 = 最新可信状态
        回写状态 = 最新可信状态 or 当前状态
        if 最新可信状态 is not None and not 是否text:
